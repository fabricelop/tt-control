...
export default {async scheduled(_event:ScheduledEvent,env:Env,ctx:ExecutionContext){ctx.waitUntil(runMediaRadar(env))},async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/api/media-radar-work'){const q=await env.DB.prepare("SELECT id,title,url,source,sources_json,source_count,importance,reason,status,first_seen,last_seen FROM media_radar WHERE status IN ('NEW','IGNORED') ORDER BY last_seen DESC LIMIT 200").all();return Response.json({news:q.results,count:q.results.length},{headers:{'cache-control':'no-store'}})}
