...
async function openDesk(id){
  ...
  desk.classList.add('mobileOpen');desk.innerHTML='<button class="mobileClose" onclick="closeDesk()">✕ Cerrar</button><h2>Mesa de redacción</h2><div class="meta">'+fmtDate(n.published_at)+'</div><h3>'+esc(n.title)+'</h3>'+
    (n.processing_error?'<p class="urgent"><b>Error de elaboración:</b> '+esc(n.processing_error)+'</p>':'')+(d.selection_instruction?'<p style="background:#fff8db;border:1px solid #eadb91;padding:10px;border-radius:8px"><b>📝 Instrucción editorial:</b> '+esc(d.selection_instruction)+'</p>':'')+
...
async function radarAgreeAll(){
  ...
  if(!confirm('¿Confirmas que estás de acuerdo con TODOS los descartes actuales del radar? Se usarán como aprendizaje y desaparecerán de esta lista.'))return
  actionBusy=true
  try{
...
export default {async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(u.pathname==='/api/agent/pending'){
      ...
      const q=await env.DB.prepare("SELECT n.id,n.title,n.url,n.section,n.published_at,n.urgent,(SELECT value FROM editorial_feedback f WHERE f.news_id=n.id AND f.kind='rewrite_request' ORDER BY f.id DESC LIMIT 1) AS rewrite_request,(SELECT COUNT(*) FROM drafts d WHERE d.news_id=n.id) AS previous_drafts,(SELECT value FROM editorial_feedback f WHERE f.news_id=n.id AND f.kind='selection_instruction' ORDER BY f.id DESC LIMIT 1) AS selection_instruction FROM news n WHERE n.status='PROCESSING' ORDER BY n.urgent DESC,n.published_at ASC,n.id ASC LIMIT 50").all()
