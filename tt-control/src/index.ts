...
async function load(){
  const desk=document.querySelector('.detail');desk.classList.remove('mobileOpen');desk.innerHTML=''
  try{
...
function closeDesk(){const desk=document.querySelector('.detail');desk.classList.remove('mobileOpen');desk.innerHTML=''}
...
export default {async scheduled(_event:ScheduledEvent,env:Env,ctx:ExecutionContext){ctx.waitUntil(runMediaRadar(env))},async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/api/public/media-radar-stats'){const q:any=await env.DB.prepare("SELECT status,COUNT(*) n,ROUND(AVG(source_count),2) avg_sources,MAX(source_count) max_sources FROM media_radar WHERE last_seen>=datetime('now','-24 hours') GROUP BY status").all();const total=(q.results as any[]).reduce((s:number,x:any)=>s+Number(x.n||0),0);return Response.json({window_hours:24,total,groups:q.results,kept:(q.results as any[]).filter((x:any)=>['ENTRY','SENT_ENTRY','ALERTED','PREPARING','PREPARED'].includes(x.status)).reduce((s:number,x:any)=>s+Number(x.n||0),0)},{headers:{'access-control-allow-origin':'*','cache-control':'no-store'}})}
