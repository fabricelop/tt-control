...
async function load(){
  ...
  const heading=document.querySelector('.list h2');heading.textContent=currentView==='SELECTED'?'Noticias seleccionadas':'Noticias para valorar';
  news.innerHTML=d.news.length?d.news.map(function(n){return '<article class="card"><div class="meta">'+fmtDate(n.published_at)+' '+(n.urgent?'<span class="urgent">· URGENTE</span>':'')+'</div><div class="title">'+esc(n.title)+'</div><div class="actions">'+(currentView==='NEW'?'<button class="yes" onclick="act('+n.id+',&quot;SELECTED&quot;)">✓ Seleccionar</button><button class="no" onclick="act('+n.id+',&quot;DISMISSED&quot;)">✕ Desestimar</button>':'<button onclick="act('+n.id+',&quot;NEW&quot;)">↩ Devolver a Entrada</button><button class="no" onclick="act('+n.id+',&quot;DISMISSED&quot;)">✕ Desestimar</button>')+'<button class="urgencyBtn '+(n.urgent?'isUrgent':'')+'" title="'+(n.urgent?'Marcar como no urgente':'Marcar como urgente')+'" onclick="urgent('+n.id+','+(n.urgent?0:1)+')">'+(n.urgent?'↓ U':'↑ U')+'</button></div></article>'}).join(''):'<div class="empty">'+(currentView==='SELECTED'?'No hay noticias seleccionadas.':'No hay noticias pendientes.')+'</div>'
}
...
async function ingest(env:Env){
  ...
  try{
    ...
    return {ok:true,discovered,admitted,radarDismissed,checkpoint:latest||checkpoint}
