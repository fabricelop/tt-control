...
async function openDesk(id){
  ...
  desk.classList.add('mobileOpen');desk.innerHTML='<button class="mobileClose" onclick="closeDesk()">✕ Cerrar</button><h2>Mesa de redacción</h2><div class="meta">'+fmtDate(n.published_at)+'</div><h3>'+esc(n.title)+'</h3>'+
    ...
    (dr?'<h3>Versión base</h3><p>'+esc(dr.base_text||'')+'</p>'+
      ...
      '<div class="actions"><button onclick="requestRewrite('+id+',false)">↻ Nuevos remates</button><button onclick="requestRewrite('+id+',true)">✎ Explicar qué falla</button><button class="danger" onclick="discardReady('+id+')">✕ Borrar noticia</button></div>'+
