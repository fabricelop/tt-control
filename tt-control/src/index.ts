...
function login(message=''){return new Response(`<!doctype html><html lang="es"><meta name="viewport" content="width=device-width"><title>TT Control</title><link rel="manifest" href="/manifest.webmanifest"><meta name="theme-color" content="#1769e0"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-title" content="TT Control"><style>body{font-family:system-ui;background:#f4f6f9;display:grid;place-items:center;height:100vh;margin:0}form{background:white;padding:28px;border-radius:14px;box-shadow:0 8px 30px #0002;width:min(360px,88vw)}input,button{width:100%;padding:12px;margin-top:12px;box-sizing:border-box}button{background:#1769e0;color:white;border:0;border-radius:8px}</style><form method="post" action="/login"><h2>TT Control</h2><input type="password" name="password" placeholder="Contraseña" autofocus><button>Entrar</button><p>${message}</p></form></html>`,{headers:{'content-type':'text/html;charset=UTF-8','cache-control':'no-store'}})}

const HTML = `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>TT Control</title><link rel="manifest" href="/manifest.webmanifest"><meta name="theme-color" content="#1769e0"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="default"><meta name="apple-mobile-web-app-title" content="TT Control"><style>
:root{font-family:Inter,system-ui,sans-serif;color:#172033;background:#f4f6f9}*{box-sizing:border-box}body{margin:0}.top{height:68px;background:#fff;border-bottom:1px solid #dde2ea;display:flex;align-items:center;padding:0 24px;gap:18px;position:sticky;top:0}.brand{font-size:22px;font-weight:800}.manual{background:#fff;color:#1769e0;border:1px solid #1769e0;border-radius:10px;padding:11px 14px;font-weight:700}.run{margin-left:auto;background:#1769e0;color:#fff;border:0;border-radius:10px;padding:12px 18px;font-weight:700}.layout{display:grid;grid-template-columns:210px minmax(430px,1fr) minmax(360px,.9fr);min-height:calc(100vh - 68px)}nav{padding:22px 14px;border-right:1px solid #dde2ea;background:#fff}nav button{display:block;width:100%;text-align:left;border:0;background:none;padding:12px;border-radius:9px;font-size:15px}.active{background:#eaf2ff!important;color:#125bc0;font-weight:700}.list{padding:22px}.detail{padding:22px;border-left:1px solid #dde2ea;background:#fff}.card{background:#fff;border:1px solid #dde2ea;border-radius:12px;padding:15px;margin-bottom:12px}.meta{font-size:12px;color:#667085;margin-bottom:6px}.title{font-weight:700;line-height:1.35}.actions{display:flex;gap:8px;margin-top:12px;flex-wrap:wrap}.actions button{border:1px solid #cfd6e1;background:#fff;border-radius:8px;padding:8px 10px}.publish{background:#111827!important;color:#fff!important;border-color:#111827!important}.danger{background:#fff0f0!important;color:#b42318!important;border-color:#f3b7b7!important}.remateChoice{border:1px solid #dde2ea;border-radius:10px;padding:10px;margin:8px 0;background:#fafbfc}.remateChoice p{margin:6px 0}.yes{background:#eaf7ee!important}.no{background:#fff0f0!important}.urgent{color:#b42318;font-weight:800}.urgencyBtn{min-width:34px;padding:5px 7px!important;font-size:12px;font-weight:800}.urgencyBtn.isUrgent{background:#d92d20!important;color:#fff;border-color:#d92d20!important}.empty{color:#667085;padding:30px 0}.mobileClose{display:none}.radarAgree{background:#eef8f0!important}.radarDisagree{background:#fff1f0!important}@media(max-width:850px){.layout{display:block}nav{display:flex;overflow:auto;border-right:0;border-bottom:1px solid #dde2ea;padding:8px}.detail{display:none}.detail.mobileOpen{display:block;position:fixed;inset:68px 0 0 0;z-index:20;overflow:auto;padding:16px}.mobileClose{display:inline-block!important;float:right;margin-bottom:10px}.card{padding:14px}.actions button{min-height:42px}.list{padding:12px}.top{padding:0 8px;gap:7px}.brand{font-size:18px}.top #stamp{display:none}.manual{padding:9px 10px;font-size:12px}nav button{white-space:nowrap;width:auto}.run{padding:10px 12px}}
</style></head><body><header class="top"><div class="brand">TT Control</div><span id="stamp">TTiTTulares</span><button class="manual" onclick="openManual()">＋ NOTICIA</button><button class="run" onclick="runNow()">▶ EJECUTAR</button></header><main class="layout"><nav><button class="active" onclick="showView(\'READY\',this)">Listas <b id="readyCount"></b></button><button onclick="showView(\'NEW\',this)">Entrada <b id="newCount"></b></button><button onclick="showView(\'SELECTED\',this)">Seleccionadas <b id="selectedCount"></b></button><button onclick="showView(\'PROCESSING\',this)">Elaborando <b id="processingCount"></b></button><button onclick="showView('RADAR_DISMISSED',this)">Descartadas radar <b id="radarCount"></b></button></nav><section class="list"><h2>Noticias para valorar</h2><div id="news" class="empty">Cargando…</div></section><aside class="detail"><h2>Mesa de redacción</h2><p>Selecciona una noticia. La investigación, versión base y los tres remates se incorporarán en la siguiente fase.</p><p><b>Urgencia</b> y <b>relevancia editorial</b> se aprenden por separado.</p></aside></main><script>
let currentView='READY'
async function load(){
  const r=await fetch('/api/news?status='+encodeURIComponent(currentView));const d=await r.json();
  newCount.textContent=d.counts&&d.counts.NEW?'('+d.counts.NEW+')':'';
  selectedCount.textContent=d.counts&&d.counts.SELECTED?'('+d.counts.SELECTED+')':'';
  ...
  radarCount.textContent=d.counts&&d.counts.RADAR_DISMISSED?'('+d.counts.RADAR_DISMISSED+')':'';
  const heading=document.querySelector('.list h2');heading.textContent=currentView==='SELECTED'?'Noticias seleccionadas':currentView==='PROCESSING'?'Noticias elaborándose':currentView==='READY'?'Noticias listas':currentView==='RADAR_DISMISSED'?'Descartadas por el radar':'Noticias para valorar';
  news.innerHTML=d.news.length?d.news.map(function(n){return '<article class="card" onclick="openDesk('+n.id+')"><div class="meta">'+fmtDate(n.published_at)+' '+(n.urgent?'<span class="urgent">· URGENTE</span>':'')+'</div><div class="title">'+esc(n.title)+'</div><div class="actions">'+(currentView==='NEW'?'<button class="yes" onclick="event.stopPropagation();act('+n.id+',&quot;SELECTED&quot;)">✓ Seleccionar</button><button class="interesting" onclick="event.stopPropagation();act('+n.id+',&quot;INTERESTING&quot;)">≈ Interesante</button><button class="no" onclick="event.stopPropagation();act('+n.id+',&quot;DISMISSED&quot;)">✕ Desestimar</button>':currentView==='SELECTED'?'<button onclick="act('+n.id+',&quot;NEW&quot;)">↩ Devolver a Entrada</button><button class="no" onclick="event.stopPropagation();act('+n.id+',&quot;DISMISSED&quot;)">✕ Desestimar</button>':currentView==='PROCESSING'?'<button onclick="event.stopPropagation();processingAct('+n.id+',&quot;NEW&quot;)">↩ Devolver a Entrada</button><button class="no" onclick="event.stopPropagation();processingAct('+n.id+',&quot;DISMISSED&quot;)">✕ Borrar</button>':currentView==='READY'?'<button class="danger" onclick="event.stopPropagation();discardReady('+n.id+')">✕ Borrar</button>':currentView==='RADAR_DISMISSED'?'<button class="radarAgree" onclick="event.stopPropagation();radarFeedback('+n.id+',1)">✓ De acuerdo</button><button class="radarDisagree" onclick="event.stopPropagation();radarFeedback('+n.id+',0)">↩ No estoy de acuerdo</button>':'<span class="meta">✓ Publicada</span>')+'<button class="urgencyBtn '+(n.urgent?'isUrgent':'')+'" title="'+(n.urgent?'Marcar como no urgente':'Marcar como urgente')+'" onclick="event.stopPropagation();urgent('+n.id+','+(n.urgent?0:1)+')">'+(n.urgent?'↓ U':'↑ U')+'</button></div></article>'}).join(''):'<div class="empty">'+(currentView==='SELECTED'?'No hay noticias seleccionadas.':currentView==='PROCESSING'?'No hay noticias elaborándose.':currentView==='READY'?'No hay noticias listas.':currentView==='RADAR_DISMISSED'?'No hay descartes automáticos pendientes de revisar.':'No hay noticias pendientes.')+'</div>'
}
...
function openManual(){
  const desk=document.querySelector('.detail');desk.classList.add('mobileOpen');desk.innerHTML='<button class="mobileClose" onclick="closeDesk()">✕ Cerrar</button><h2>＋ Añadir noticia</h2><p class="meta">Pega un enlace, un titular o ambos. Entrará directamente en Elaborando.</p><input id="manualUrl" placeholder="Enlace (opcional)" style="width:100%;padding:11px;margin:6px 0;border:1px solid #cfd6e1;border-radius:8px"><textarea id="manualTitle" placeholder="Titular o lo que sabes de la noticia" style="width:100%;min-height:100px;padding:11px;margin:6px 0;border:1px solid #cfd6e1;border-radius:8px"></textarea><div class="actions"><button class="publish" onclick="submitManual()">Preparar noticia</button></div>'
}
async function submitManual(){
  ...
  const r=await fetch('/api/news/manual',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({url:url,title:title})});const d=await r.json();if(!r.ok)return alert(d.error||'No se pudo añadir')
...
function closeDesk(){document.querySelector('.detail').classList.remove('mobileOpen')}
...
async function ingest(env:Env){
  ...
  try{
    const claimed=await env.DB.prepare("UPDATE news SET status='PROCESSING',processing_started_at=datetime('now'),processing_error=NULL,updated_at=datetime('now') WHERE status='SELECTED'").run()
...
export default {async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='POST'&&u.pathname==='/api/news/ai-image'){
      ...
      const prompt='Witty satirical editorial cartoon for a Spanish digital news account. Topic: '+String(d.base_text||n.title)+'. Exaggerated visual metaphor, playful absurdity, sharp newspaper-cartoon humor, expressive simplified characters or symbolic objects, bold shapes, energetic composition, illustrated screenprint/comic feel, unmistakably drawn and not photorealistic. No written words, no logos, no fake screenshot, no depiction that could be mistaken for documentary evidence. Leave clean space at the bottom for a publisher brand strip.'
      try{
    ...
    if(req.method==='POST'&&u.pathname==='/api/news/radar-feedback'){const b:any=await req.json(),id=Number(b.id),agree=!!b.agree;if(!id)return Response.json({error:'id inválido'},{status:400});const n=await env.DB.prepare("SELECT status FROM news WHERE id=?").bind(id).first<{status:string}>();if(!n||n.status!=='RADAR_DISMISSED')return Response.json({error:'La noticia no está en descartes del radar'},{status:409});const status=agree?'DISMISSED':'NEW',value=agree?'AGREE':'DISAGREE';await env.DB.batch([env.DB.prepare("UPDATE news SET status=?,updated_at=datetime('now') WHERE id=?").bind(status,id),env.DB.prepare("INSERT INTO editorial_feedback(news_id,kind,value,created_at) VALUES(?,'radar_feedback',?,datetime('now'))").bind(id,value)]);return Response.json({ok:true,status})}
