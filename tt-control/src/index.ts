...
export default {async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/api/news'){const q=await env.DB.prepare("SELECT id,title,url,section,published_at,urgent FROM news WHERE status='NEW' ORDER BY published_at ASC, id ASC LIMIT 200").all();return Response.json({news:q.results})}
