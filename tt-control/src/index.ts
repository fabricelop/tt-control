...
async function load(){
  ...
  const heading=document.querySelector('.list h2');heading.textContent=currentView==='SELECTED'?'Noticias seleccionadas':currentView==='PROCESSING'?'Noticias elaborándose':currentView==='READY'?'Noticias listas':'Noticias para valorar';
  news.innerHTML=d.news.length?d.news.map(function(n){return '<article class="card" onclick="openDesk('+n.id+')"><div class="meta">'+fmtDate(n.published_at)+' '+(n.urgent?'<span class="urgent">· URGENTE</span>':'')+'</div><div class="title">'+esc(n.title)+'</div><div class="actions">'+(currentView==='NEW'?'<button class="yes" onclick="event.stopPropagation();act('+n.id+',&quot;SELECTED&quot;)">✓ Seleccionar</button><button class="no" onclick="act('+n.id+',&quot;DISMISSED&quot;)">✕ Desestimar</button>':currentView==='SELECTED'?'<button onclick="act('+n.id+',&quot;NEW&quot;)">↩ Devolver a Entrada</button><button class="no" onclick="act('+n.id+',&quot;DISMISSED&quot;)">✕ Desestimar</button>':currentView==='PROCESSING'?'<span class="meta">En elaboración…</span>':'<button class="danger" onclick="event.stopPropagation();discardReady('+n.id+')">✕ Borrar</button>')+'<button class="urgencyBtn '+(n.urgent?'isUrgent':'')+'" title="'+(n.urgent?'Marcar como no urgente':'Marcar como urgente')+'" onclick="event.stopPropagation();urgent('+n.id+','+(n.urgent?0:1)+')">'+(n.urgent?'↓ U':'↑ U')+'</button></div></article>'}).join(''):'<div class="empty">'+(currentView==='SELECTED'?'No hay noticias seleccionadas.':currentView==='PROCESSING'?'No hay noticias elaborándose.':currentView==='READY'?'No hay noticias listas.':'No hay noticias pendientes.')+'</div>'
}
...
export default {async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/api/news'){const requested=u.searchParams.get('status')||'NEW';const status=['NEW','SELECTED','PROCESSING','READY'].includes(requested)?requested:'NEW';const q=await env.DB.prepare("SELECT id,title,url,section,published_at,urgent FROM news WHERE status=? ORDER BY urgent DESC, published_at ASC, id ASC LIMIT 200").bind(status).all();const counts=await env.DB.prepare("SELECT status,COUNT(*) AS n FROM news WHERE status IN ('NEW','SELECTED','PROCESSING','READY') GROUP BY status").all();const c:any={NEW:0,SELECTED:0,PROCESSING:0,READY:0};for(const row of counts.results as any[])c[row.status]=row.n;return Response.json({news:q.results,counts:c})}
