...
export default {async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    if(req.method==='GET'&&u.pathname==='/manifest.webmanifest')return Response.json({name:'TT Control',short_name:'TT Control',start_url:'/',display:'standalone',background_color:'#f4f6f9',theme_color:'#1769e0',icons:[]},{headers:{'content-type':'application/manifest+json'}})
