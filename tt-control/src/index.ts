...
async function telegramApi(env:Env,method:string,body:any){let token=env.TELEGRAM_BOT_TOKEN||'';if(!token){const s:any=await env.DB.prepare("SELECT value FROM app_settings WHERE key='telegram_bot_token'").first();token=String(s?.value||'')}if(!token)return {ok:false,skipped:true};const r=await fetch('https://api.telegram.org/bot'+token+'/'+method,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});if(!r.ok)return {ok:false,status:r.status};return r.json()}
...
let currentView=new URLSearchParams(location.search).get('view')||'READY'
let openNewsId=0
...
function showView(view,btn){openNewsId=0;currentView=view;document.querySelectorAll('nav button').forEach(function(x){x.classList.remove('active')});btn.classList.add('active');load()}
...
async function runMediaRadar(env:Env){
  ...
  for(const x of items){const fp=mediaFingerprint(x.title);if(fp.length<12)continue;const similar:any=await env.DB.prepare("SELECT * FROM media_radar WHERE status IN ('NEW','ENTRY','ALERTED','IGNORED') AND last_seen>=datetime('now','-18 hours') ORDER BY last_seen DESC LIMIT 120").all();let row:any=null
    const toks=new Set(fp.split('|'));let best:any=null,bestOverlap=0;for(const r of similar.results as any[]){const rt=new Set(String(r.fingerprint).split('|'));const overlap=[...toks].filter(t=>rt.has(t)).length/Math.max(1,Math.min(toks.size,rt.size));if(overlap>bestOverlap){bestOverlap=overlap;best=r}if(overlap>=.48){row=r;break}}
...
export default {async scheduled(_event:ScheduledEvent,env:Env,ctx:ExecutionContext){ctx.waitUntil(runMediaRadar(env))},async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/api/media-radar-work'){const q:any=await env.DB.prepare("SELECT id,title,url,source,sources_json,source_count,importance,reason,status,first_seen,last_seen FROM media_radar WHERE last_seen>=datetime('now','-12 hours') AND status IN ('NEW','IGNORED','ENTRY','SENT_ENTRY','ALERTED','PREPARING') ORDER BY last_seen DESC LIMIT 300").all();const rows=q.results as any[];const defs=[['EVALUATING','Evaluando','Aún pendiente de clasificación'],['IGNORE','Descartables','Evaluadas como poco relevantes'],['ENTRY','Candidatas a Entrada','Revisión humana sin alerta móvil'],['ALERT','Candidatas a Telegram','Importancia alta / alerta móvil'],['PROMOTED','Ya promovidas','Enviadas a Entrada o Elaborando']];const bucket=(r:any)=>r.status==='NEW'?'EVALUATING':r.status==='IGNORED'?'IGNORE':r.status==='ENTRY'?'ENTRY':r.status==='ALERTED'?'ALERT':'PROMOTED';const groups=defs.map(d=>({key:d[0],label:d[1],description:d[2],news:rows.filter(r=>bucket(r)===d[0])}));return Response.json({groups,count:rows.length},{headers:{'cache-control':'no-store'}})}
    ...
    if(req.method==='GET'&&u.pathname==='/api/public/ready-feed'){const q:any=await env.DB.prepare("SELECT n.id,n.title,n.url,n.status,d.base_text,d.remate_a,d.remate_b,d.remate_c,d.version,d.updated_at FROM news n JOIN drafts d ON d.news_id=n.id AND d.version=(SELECT MAX(d2.version) FROM drafts d2 WHERE d2.news_id=n.id) WHERE n.status NOT IN ('DISMISSED','RADAR_DISMISSED') ORDER BY d.updated_at DESC LIMIT 40").all();return Response.json({ready:q.results},{headers:{'access-control-allow-origin':'*','cache-control':'no-store'}})}
    if(req.method==='POST'&&u.pathname==='/api/telegram-webhook'){const secret=req.headers.get('x-telegram-bot-api-secret-token')||'';if(env.TELEGRAM_WEBHOOK_SECRET&&secret!==env.TELEGRAM_WEBHOOK_SECRET)return new Response('Forbidden',{status:403});const update:any=await req.json();const cq=update?.callback_query;if(!cq)return Response.json({ok:true});const data=String(cq.data||''),msg=cq.message||{},chat=String(msg?.chat?.id||'');let allowedChat=String(env.TELEGRAM_CHAT_ID||'');if(!allowedChat){const cs:any=await env.DB.prepare("SELECT value FROM app_settings WHERE key='telegram_chat_id'").first();allowedChat=String(cs?.value||'')}if(allowedChat&&chat!==allowedChat)return Response.json({ok:true});if(data.startsWith('media:')){const p=data.split(':'),action=p[1]||'',id=Number(p[2]);if(!['PREPARE','INTERESTING','DISMISS'].includes(action)||!Number.isInteger(id)){await telegramApi(env,'answerCallbackQuery',{callback_query_id:cq.id,text:'Acción no válida.'});return Response.json({ok:true})}const n:any=await env.DB.prepare("SELECT * FROM media_radar WHERE id=?").bind(id).first();if(!n){await telegramApi(env,'answerCallbackQuery',{callback_query_id:cq.id,text:'Noticia no encontrada.'});return Response.json({ok:true})}if(action==='PREPARE'){const key='media:'+id;await env.DB.prepare("INSERT INTO news(source_key,title,url,section,published_at,detected_at,status,radar_reason,updated_at) VALUES(?,?,?,?,datetime('now'),datetime('now'),'PROCESSING','Seleccionada desde Radar',datetime('now')) ON CONFLICT(source_key) DO UPDATE SET status='PROCESSING',updated_at=datetime('now')").bind(key,n.title,n.url,'Radar').run();await env.DB.prepare("UPDATE media_radar SET status='PREPARING' WHERE id=?").bind(id).run()}else await env.DB.prepare("UPDATE media_radar SET status=? WHERE id=?").bind(action==='INTERESTING'?'INTERESTING':'DISMISSED',id).run();await telegramApi(env,'answerCallbackQuery',{callback_query_id:cq.id,text:action==='PREPARE'?'Enviada a Elaborando.':action==='INTERESTING'?'Marcada como interesante.':'Descartada.'});if(msg.message_id)await telegramApi(env,'deleteMessage',{chat_id:msg.chat.id,message_id:msg.message_id});return Response.json({ok:true,status:action})}return Response.json({ok:true})}
    ...
    if(req.method==='GET'&&u.pathname==='/api/public/media-alerts-feed'){const q=await env.DB.prepare("SELECT id,title,url,source,sources_json,source_count,reason,last_seen FROM media_radar WHERE status='ALERTED' ORDER BY last_seen DESC LIMIT 20").all();return Response.json({alerts:q.results},{headers:{'access-control-allow-origin':'*','cache-control':'no-store'}})}
    if(req.method==='GET'&&u.pathname==='/api/public/media-test-candidate'){const n:any=await env.DB.prepare("SELECT id,title,url,source,sources_json,source_count,reason,last_seen FROM media_radar WHERE status IN ('NEW','ENTRY','IGNORED') ORDER BY last_seen DESC LIMIT 1").first();return Response.json({candidate:n||null},{headers:{'access-control-allow-origin':'*','cache-control':'no-store'}})}
