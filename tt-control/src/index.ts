...
async function ingest(env:Env){
  ...
  try{
    const claimed=await env.DB.prepare("UPDATE news SET status='PROCESSING',updated_at=datetime('now') WHERE status='SELECTED'").run()
