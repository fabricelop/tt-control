...
export default {async scheduled(_event:ScheduledEvent,env:Env,ctx:ExecutionContext){ctx.waitUntil((async()=>{const guard:any=await env.DB.prepare("SELECT value FROM app_settings WHERE key='radar_cron_tick'").first().catch(()=>null);const tick=Number(guard?.value||0)+1;await env.DB.prepare("INSERT INTO app_settings(key,value,updated_at) VALUES('radar_cron_tick',?,datetime('now')) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=datetime('now')").bind(String(tick)).run();if(tick%3===0)await ingest(env)})())},async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(u.pathname==='/api/agent/pending'){
      ...
      const q=await env.DB.prepare("SELECT n.id,n.title,n.url,n.section,n.published_at,(SELECT value FROM editorial_feedback f WHERE f.news_id=n.id AND f.kind='rewrite_request' ORDER BY f.id DESC LIMIT 1) AS rewrite_request,(SELECT COUNT(*) FROM drafts d WHERE d.news_id=n.id) AS previous_drafts,(SELECT value FROM editorial_feedback f WHERE f.news_id=n.id AND f.kind='selection_instruction' ORDER BY f.id DESC LIMIT 1) AS selection_instruction FROM news n WHERE n.status='PROCESSING' ORDER BY n.published_at ASC,n.id ASC LIMIT 50").all()
      let radar:RadarModel={pos:0,neg:0,pc:new Map(),nc:new Map()};let rc:any={results:[]};let ci:any=null
      try{radar=await buildRadarModel(env)}catch{}
      try{rc=await env.DB.prepare("SELECT status,COUNT(*) AS n FROM news GROUP BY status").all()}catch{}
      try{ci=await env.DB.prepare("SELECT value,updated_at FROM app_settings WHERE key='control_instruction'").first()}catch{}
      return Response.json({news:q.results,control_instruction:ci||null,editorial_instructions:{remates:"OBLIGATORIO por defecto en todos los tuits que admitan humor: tres remates realmente humorísticos y específicos de la noticia. Deben ser burlescos, satíricos, irónicos o de humor negro/fino según el contexto; buscar contradicciones, hemeroteca, protagonistas, consecuencias y detalles verificados que permitan un giro inesperado. No usar como remate una continuación informativa del titular, una frase hecha, una obviedad ni una paráfrasis. Investigar también para encontrar el ángulo cómico más fuerte. Al menos una alternativa debe ser claramente más mordaz. En política, basar la sátira en hechos/contradicciones documentados sin pedir el voto ni convertirla en propaganda partidista. SIEMPRE entregar tres remates, también en política y en noticias con víctimas. En noticias con víctimas, el remate nunca debe bromear sobre las víctimas ni su sufrimiento: dirigir la sátira, cuando sea viable, a responsables, instituciones, contradicciones, propaganda, explicaciones oficiales o contexto. Si el asunto es extremadamente sensible, usar ironía sobria dirigida al contexto, pero nunca escribir marcadores como 'Sin remate'.",length:"Cada variante base + 🌶️ + remate debe caber en 280 caracteres antes de entregarla."},radar:{positive_samples:radar.pos,negative_samples:radar.neg,minimum_positive:5,minimum_negative:20,learning_active:radar.pos>=5&&radar.neg>=20,status_counts:Object.fromEntries((rc.results as any[]).map(x=>[String(x.status),Number(x.n)]))}})
    }
