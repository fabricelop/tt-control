...
</style></head><body><header class="top"><div class="brand">TT Control</div><span id="stamp">TTiTTulares</span><button class="manual" onclick="openManual()">＋ NOTICIA</button><span id="autoCountdown" class="meta" title="Próxima ejecución automática">Auto · 05:00</span><label class="meta" title="1 = más noticias · 5 = menos noticias">Radar <select id="radarSensitivity" onchange="setRadarSensitivity(this.value)"><option value="1">1 · Máximo</option><option value="2">2 · Alto</option><option value="3" selected>3 · Normal</option><option value="4">4 · Bajo</option><option value="5">5 · Mínimo</option></select></label><button class="run" onclick="runNow()">RADAR</button></header><main class="layout"><nav><button class="active" onclick="showView(\'READY\',this)">Listas <b id="readyCount"></b></button><button onclick="showView(\'NEW\',this)">Entrada <b id="newCount"></b></button><button onclick="showView(\'PROCESSING\',this)">Elaborando <b id="processingCount"></b></button></nav><section class="list"><h2>Noticias para valorar</h2><div id="news" class="empty">Cargando…</div></section><aside class="detail"></aside></main><script>
...
async function runMediaRadar(env:Env){
  const setting=await env.DB.prepare("SELECT value FROM app_settings WHERE key='radar_sensitivity'").first<{value:string}>();const sensitivity=Math.max(1,Math.min(5,Number(setting?.value||3)));const entryCoverage=[0,.12,.22,.33,.44,.55][sensitivity],alertCoverage=[0,.34,.45,.67,.78,.89][sensitivity]
...
export default {async scheduled(_event:ScheduledEvent,env:Env,ctx:ExecutionContext){ctx.waitUntil(runMediaRadar(env))},async fetch(req:Request,env:Env):Promise<Response>{
  ...
  try{
    ...
    if(req.method==='GET'&&u.pathname==='/api/public/ready-feed'){const q:any=await env.DB.prepare("SELECT n.id,n.title,n.url,d.base_text,d.remate_a,d.remate_b,d.remate_c,d.version,d.updated_at FROM news n JOIN drafts d ON d.news_id=n.id AND d.version=(SELECT MAX(d2.version) FROM drafts d2 WHERE d2.news_id=n.id) WHERE n.status='READY' ORDER BY n.updated_at DESC LIMIT 20").all();return Response.json({ready:q.results},{headers:{'access-control-allow-origin':'*','cache-control':'no-store'}})}
